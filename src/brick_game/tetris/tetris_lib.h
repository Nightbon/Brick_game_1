...
typedef enum {
  ...
  Right,      ///< Вправо
  Down,       ///< Вниз
  Action  ///< Специальное действие (например, поворот)
} User_action_t;
...
/**
 * @brief Структура для отображения информации о состоянии игры (для UI).
 */
typedef struct {
  int field[20][10];  ///< Поле с расположением блоков
  int next_figure[4][4];  ///< Матрица следующей фигуры
  int hi_score;           ///< Рекорд
  int score;              ///< Счет
  int lvl;                ///< Уровень
  int pause;              ///< Пауза
 ...
 * @param action Действие пользователя.
 */
void user_input(User_action_t action);
