...
void move_fig(Game_intro *val, User_action_t action) {
  ...
  if (now - val->last_time >= val->delay_ms - (val->lvl * 20)) {
    val->last_time = now;
...
void load_high_score(Game_intro *val) {
  ...
  if (file) {
    fread(&val->hi_score, sizeof(int), 1, file);
    fclose(file);
  }
}
...
void calc_score(Game_intro *val) {
  ...
  val->lvl = val->score / 600;
  val->lvl = val->lvl > 10 ? 10 : val->lvl;
  if (val->score > val->hi_score) {
    save_high_score(val);
    val->hi_score = val->score;
  }
}
...
Game_intro core(User_action_t action) {
  ...
  if (val.status == Spawn && action != Empty) {
    spawn(&val);
    val.status = Move_fig;
  }
  if (val.status == Move_fig && action != Empty) {
    move_fig(&val, action);
  }
  if (val.status == Calc_score && action != Empty) {
    calc_score(&val);
...
void user_input(User_action_t action) { core(action); }

Game_info_t get_game_info() {
  Game_intro tmp = core(Empty);
  Game_info_t result = {0};
  for (int i = 0; i < 20; i++) {
    for (int j = 0; j < 10; j++) {
      int is_figure_here = (i >= tmp.fig.y && i < tmp.fig.y + 4 &&
                            j >= tmp.fig.x && j < tmp.fig.x + 4);
      if (tmp.field[i][j] ||
          (is_figure_here && tmp.fig.field[i - tmp.fig.y][j - tmp.fig.x])) {
        result.field[i][j] = 1;
      }
    }
  }
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      result.next_figure[i][j] = tmp.next_fig.field[i][j];
    }
  }

  result.hi_score = tmp.hi_score;
  result.lvl = tmp.lvl;
